...
describe("Ticket", function () {
  ...
  beforeEach(async function () {
    ...
    ticket = await Ticket.deploy("My Concert", "Artist Name", "Venue Name", Date.now() / 1000 + 3600, Date.now() / 1000 + 7200, factory.address, eventCreator.address);
  ...
  it("Should allow the factory to mint new tickets", async function () {
    await expect(ticket.connect(factory).mint(owner.address, "VIP", ethers.utils.parseEther("1"), "A1"))
      .to.emit(ticket, "TicketMinted")
      .withArgs(0, owner.address, "VIP", ethers.utils.parseEther("1"), "A1");
    expect(await ticket.ownerOf(0)).to.equal(owner.address);
    expect(await ticket.category(0)).to.equal("VIP");
    expect(await ticket.price(0)).to.equal(ethers.utils.parseEther("1"));
    expect(await ticket.seatNumber(0)).to.equal("A1");
  });

  it("Should prevent non-factory addresses from minting tickets", async function () {
    await expect(ticket.mint(owner.address, "General", ethers.utils.parseEther("0.5"), "")).to.be.revertedWith(
      "Only the ticket factory can call this function."
    );
  });

  it("Should allow the factory to mark a ticket as used", async function () {
    await ticket.connect(factory).mint(owner.address, "General", ethers.utils.parseEther("0.5"), "");
